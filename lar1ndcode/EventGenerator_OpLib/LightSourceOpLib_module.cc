// \file LightSourceOpLib_module.cxx  - Ben Jones, MIT 2010
//
...
//
#ifndef EVGEN_LIGHTSOURCEOL_H
#define EVGEN_LIGHTSOURCEOL_H
...
namespace evgen {
  ...
  class LightSourceOpLib : public art::EDProducer {
  public:
    explicit LightSourceOpLib(fhicl::ParameterSet const& pset);
    virtual ~LightSourceOpLib();                        
...
namespace evgen{
  ...
  //----------------------------------------------------------------
  LightSourceOpLib::LightSourceOpLib(fhicl::ParameterSet const& pset) 
  {
	...
	mf::LogVerbatim("LightSourceOpLib") << "Light Source : Determining voxel params : " 
				       << fVoxelCount << " " 
				       << fSigmaX     << " "
				       << fSigmaY     << " " 
				       <<fSigmaZ;
      ...
      throw cet::exception("LightSourceOpLib") << "EVGEN Light Source : Unrecognised light source mode\n";
    }
  ...
  //----------------------------------------------------------------
  LightSourceOpLib::~LightSourceOpLib()
  {
  }

  //____________________________________________________________________________
  void LightSourceOpLib::beginRun(art::Run& run)
  {
  ...
  //----------------------------------------------------------------
  void LightSourceOpLib::produce(art::Event& evt)
  {
	...
	if(fInputFile.eof()){
	  mf::LogWarning("LightSourceOpLib") << "EVGEN Light Source : Warning, reached end of file,"
	...
	if(!fInputFile.is_open() || fInputFile.fail() ){
	  throw cet::exception("LightSourceOpLib") << "EVGEN Light Source : File error in " 
					      << fFileName << "\n";
	}
	else{ 
	  // read in one line
    ...
    //  - neither file or scan mode, probably a config file error
      ...
      throw cet::exception("LightSourceOpLib") <<"EVGEN : Light Source, unrecognised source mode\n";
    }
    ...
    if(vis->IsBuildJob())
      {
	mf::LogVerbatim("LightSourceOpLib") << "Light source : Stowing voxel params ";
	...
	++fCurrentVoxel;
      ...
      {
	mf::LogVerbatim("LightSourceOpLib") << "EVGEN Light Source fully scanned detector.  Starting over.";
	fCurrentVoxel=fFirstVoxel;
      }
  ...
  void LightSourceOpLib::Sample(simb::MCTruth& mct) 
  {
    mf::LogVerbatim("LightSourceOpLib") <<"Light source debug : Shooting at " << fX <<" " << fY<<" "<< fZ;
...
namespace evgen{
  ...
  DEFINE_ART_MODULE(LightSourceOpLib)

}//end namespace evgen


#endif // EVGEN_LIGHTSOURCEOL_H
////////////////////////////////////////////////////////////////////////
