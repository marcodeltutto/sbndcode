...
// LArSoft Includes
#include "lar1ndcode/LArG4_OpLib/LArVoxelReadoutGeometry.h"
#include "lar1ndcode/LArG4_OpLib/PhysicsList.h"
#include "lar1ndcode/LArG4_OpLib/ParticleListAction.h"
#include "lar1ndcode/LArG4_OpLib/LaRLightEnergyAction.h"
#include "lar1ndcode/LArG4_OpLib/IonizationAndScintillationAction.h"
#include "lar1ndcode/LArG4_OpLib/OpDetSensitiveDetector.h"
#include "lar1ndcode/LArG4_OpLib/OpDetReadoutGeometry.h"
#include "lar1ndcode/LArG4_OpLib/LArStackingAction.h"
#include "lar1ndcode/LArG4_OpLib/LArVoxelReadout.h"
#include "lar1ndcode/LArG4_OpLib/MaterialPropertyLoader.h"
...
namespace larg4 {  
  ...
  class LArG4OpLib : public art::EDProducer{
  public:
 
    /// Standard constructor and destructor for an FMWK module.
    explicit LArG4OpLib(fhicl::ParameterSet const& pset);
    virtual ~LArG4OpLib();
...
namespace larg4 {
  ...
  //----------------------------------------------------------------------
  // Constructor
  LArG4OpLib::LArG4OpLib(fhicl::ParameterSet const& pset)
    : fG4Help                (0)
  ...
  //----------------------------------------------------------------------
  // Destructor
  LArG4OpLib::~LArG4OpLib()
  {
    if(fG4Help) delete fG4Help;
  }

  //----------------------------------------------------------------------
  void LArG4OpLib::beginJob()
  {
  ...
  void LArG4OpLib::produce(art::Event& evt)

  {

    LOG_DEBUG("lar1ndcode/LArG4_OpLib") << "produce()";
    ...
    for(size_t mcl = 0; mcl < mclists.size(); ++mcl){
      ...
      for(size_t m = 0; m < mclistHandle->size(); ++m){
        ...
        LOG_DEBUG("lar1ndcode/LArG4_OpLib") << *(mct.get());
        ...
        if (fdumpParticleList){
          ...
          mf::LogInfo("lar1ndcode/LArG4_OpLib") << "Dump sim::ParticleList; size()="
    ...
    for(unsigned int c = 0; c < geom->Ncryostats(); ++c){
      ...
      for(unsigned int t = 0; t < ntpcs; ++t){
        ...
        if ( !sd ){
          ...
          throw cet::exception("lar1ndcode/LArG4_OpLib") << "Sensitive detector for cryostat "
        ...
        if ( !larVoxelReadout ){
          ...
          throw cet::exception("lar1ndcode/LArG4_OpLib") << "Sensitive detector '"
        ...
        if (!channels.empty()) {
          ...
          LOG_DEBUG("lar1ndcode/LArG4_OpLib") << "now put " << channels.size() << " SimChannels"
    ...
    for(unsigned int a = 0; a < geom->NAuxDets(); ++a){
      ...
      LOG_DEBUG("lar1ndcode/LArG4_OpLib") << "now put the AuxDetSimTracks in the event";
