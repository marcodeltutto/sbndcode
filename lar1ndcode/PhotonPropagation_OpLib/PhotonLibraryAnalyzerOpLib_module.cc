#ifndef PHOTLIBANALYZEROL_H
#define PHOTLIBANALYZEROL_H

//
// Name: PhotonLibraryAnalyzerOpLib.h
//
//
...
namespace phot {
  ...
  class PhotonLibraryAnalyzerOpLib : public art::EDAnalyzer
  {
  public:
 
    // Constructors, destructor

    explicit PhotonLibraryAnalyzerOpLib(fhicl::ParameterSet const& pset);
    virtual ~PhotonLibraryAnalyzerOpLib();
...
namespace phot {
  ...
  //----------------------------------------------------------------------------
  PhotonLibraryAnalyzerOpLib::PhotonLibraryAnalyzerOpLib(const fhicl::ParameterSet& pset)
    : EDAnalyzer(pset)
  {
    reconfigure(pset);
  }
  ...
  //----------------------------------------------------------------------------
  PhotonLibraryAnalyzerOpLib::~PhotonLibraryAnalyzerOpLib()
  {
  }

  //----------------------------------------------------------------------------
  void PhotonLibraryAnalyzerOpLib::reconfigure(fhicl::ParameterSet const& pset)
  {
      fAltXAxis = pset.get<std::string>("alt_x_axis");
  }

  //----------------------------------------------------------------------------
  void PhotonLibraryAnalyzerOpLib::beginJob()
  
  {
    mf::LogInfo("PhotonLibraryAnalyzerOpLib")<<"Analyzing photon library - begin"<<
      std::endl;

    
    art::ServiceHandle<art::TFileService> tfs;
    art::ServiceHandle<PhotonVisibilityServiceOpLib> pvs;
    ...
    mf::LogInfo("PhotonLibraryAnalyzerOpLib") << "UpperCorner: " << UpperCorner[0] << " " << UpperCorner[1] << " " << UpperCorner[2] << "\n"
   ...
   // TH3D *FullVolume = tfs->make<TH3D>("FullVolume","FullVolume", 
    ...
    mf::LogInfo("PhotonLibraryAnalyzerOpLib")<<"Analyzing photon library - making historams"<<
      std::endl;
  ...
  mf::LogInfo("PhotonLibraryAnalyzerOpLib")<<"Analyzing photon library - running through voxels "<<
    std::endl;
  ...
  mf::LogInfo("PhotonLibraryAnalyzerOpLib")<<"Analyzing photon library - end"<<
    std::endl;
  }

  //----------------------------------------------------------------------------
  void PhotonLibraryAnalyzerOpLib::analyze(const art::Event& /*evt*/)
  {
  ...
  //----------------------------------------------------------------------------
  void PhotonLibraryAnalyzerOpLib::endJob()
  {
...
namespace phot {
  DEFINE_ART_MODULE(PhotonLibraryAnalyzerOpLib)
}
