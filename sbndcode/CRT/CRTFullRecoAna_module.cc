...
#include "sbndcode/CRT/CRTUtils/CRTGeoAlg.h"
...
namespace sbnd {
  ...
  void CRTFullRecoAna::beginJob()
  {
    ...
    for(size_t i = 0; i < crtGeo.NumTaggers(); i++){
      std::string taggerName = crtGeo.GetTagger(i).name;
  ...
  void CRTFullRecoAna::analyze(const art::Event& event)
  {
    ...
    double driftTimeMuS = tpcGeo.MaxX()/fDetectorProperties->DriftVelocity(); // [us]
    ...
    for (auto const& particle: (*particleHandle)){
      ...
      for(size_t i = 0; i < crtGeo.NumTaggers(); i++){
        ...
        geo::Point_t trueCP = crtGeo.TaggerCrossingPoint(taggerName, particle);
        // Make map for each tagger with true ID as key and value an XYZ position
        if(trueCP.X() != -99999){ 
          truthMatch.trueCrosses[taggerName] = trueCP;
        }
        bool validCP = crtGeo.ValidCrossingPoint(taggerName, particle);
    ...
    for(auto const& truthMatch : truthMatching){
      ...
      // - Enters CRT volume
      bool entersCRT = crtGeo.EntersVolume(particles[partID]);
      ...
      // - Enters TPC volume
      bool entersTPC = tpcGeo.EntersVolume(particles[partID]);
      ...
      // - Crosses CRT volume
      bool crossesCRT = crtGeo.CrossesVolume(particles[partID]);
      ...
      // - Crosses TPC volume
      bool crossesTPC = tpcGeo.CrossesVolume(particles[partID]);
