...
namespace sbnd {
  ...
  void StoppingParticle::analyze(const art::Event& event)
  {
    ...
    for (auto const& particle: (*particleHandle)){
      ...
      bool sif = CosmicRemovalUtils::InFiducial(start, 0, 0);
      geo::Point_t end {particle.EndX(), particle.EndY(), particle.EndZ()};
      bool eif = CosmicRemovalUtils::InFiducial(end, 0, 0);
      if((sif && !eif) || (eif && !sif)){
    ...
    for(auto const& tpcTrack : (*tpcTrackHandle)){
      ...
      if(CosmicRemovalUtils::InFiducial(trueStart, 0., 0.)){
      ...
      if(CosmicRemovalUtils::InFiducial(trueEnd, 0., 0.)){
