...
namespace geo{
  ...
  //----------------------------------------------------------------------------
  ChannelMapsbndAlg::ChannelMapsbndAlg(fhicl::ParameterSet const& p)
    : fSorter(geo::GeoObjectSortersbnd(p))
  {
  }

  //----------------------------------------------------------------------------
  ChannelMapsbndAlg::~ChannelMapsbndAlg()
  {
  }

  //----------------------------------------------------------------------------
  void ChannelMapsbndAlg::Initialize(std::vector<geo::CryostatGeo*> & cgeo,
    ...
    for(unsigned int c = 0; c != fNcryostat; ++c){
      for(unsigned int a = 0; a != fNTPC[c]; ++a){
        for(unsigned int p = 0; p != fPlanesPerAPA; ++p){
		  ...
		  //was originally t=2*a for LBNE--no anchored vs unanchored wires for SBND 
          unsigned int t = a;
  ...
  //----------------------------------------------------------------------------
  void ChannelMapsbndAlg::Uninitialize()
  {
  ...
  //----------------------------------------------------------------------------
  std::vector<geo::WireID> ChannelMapsbndAlg::ChannelToWire(uint32_t channel)  const
  {
...
//	std::cout<<"NUMBER OF CHANNELS: "<<channel<<", "<<fNchannels<<std::endl;
  ...
  */
  ...
  //----------------------------------------------------------------------------
  uint32_t ChannelMapsbndAlg::Nchannels() const
  {
    return fNchannels;
  }
  ...
  //----------------------------------------------------------------------------
  double ChannelMapsbndAlg::WireCoordinate(double YPos, double ZPos,
					  ...
					  unsigned int cstat) const
{
...
}  // ChannelMarsbndAlf::WireCoordinate()
  

  //----------------------------------------------------------------------------
  WireID  ChannelMapsbndAlg::NearestWireID(const TVector3& xyz,
					 unsigned int    plane,
					 unsigned int    tpc,
  ...
  //----------------------------------------------------------------------------
  uint32_t ChannelMapsbndAlg::PlaneWireToChannel(unsigned int plane,
					       unsigned int wire,
					       unsigned int tpc,
					       unsigned int cstat) const
  {
...
//	std::cout<<"WIRE  IS :"<<wire<<std::endl;
  ...
  //----------------------------------------------------------------------------
  SigType_t ChannelMapsbndAlg::SignalType( uint32_t const channel )  const
  {
  ...
  //----------------------------------------------------------------------------
  View_t ChannelMapsbndAlg::View( uint32_t const channel )  const
  {
    uint32_t chan = channel % fChannelsPerAPA;
    View_t view = geo::kU;
  ...
  //----------------------------------------------------------------------------
  std::set<View_t> const& ChannelMapsbndAlg::Views() const
  {
    return fViews;
  }

  //----------------------------------------------------------------------------
  std::set<PlaneID> const& ChannelMapsbndAlg::PlaneIDs() const
  {
    return fPlaneIDs;
  }
