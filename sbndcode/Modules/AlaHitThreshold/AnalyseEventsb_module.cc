...
void MCtest::AnalyseEvents::analyze(art::Event const& e)
{
  ...
  std::map<geo::PlaneID, std::vector<float> > PeakTime_map;
  ...
  //for(auto const& hit: initialtrackhitslargest)
  {
    ...
    float PeakTimeToHits = hit->PeakTime();
    //-----------------------
    ...
    PeakTime_map[planeid].push_back(PeakTimeToHits);
  ...
  for(auto mapiter : planewhmap) //loop on plane
  {
    ...
    //wirefficiency.clear();
    //
    //--------------------looping inside the vector of PeakTime---------------------------------//
    std::vector<float> &myvector = PeakTime_map[mapiter.first];
    for(size_t i =0; i<21; ++i){
      if(i ==20){prePeakTime = prepekt/i;}
      else {prepekt += myvector[i];}
    }
    //-------------------loop inside the vecotr on hits--------------------//
    for (auto const& hit: mapiter.second)
    {
      float PeakTimeToHit = hit->PeakTime();
      ...
      if((abs(PeakTimeToHit - prePeakTime)) < 60){
        ++hitc;
        prePeakTime = PeakTimeToHit;
      {
      ...
      else{prePeakTime = PeakTimeToHit;}
