...
bool shower::anglefilter::filter(art::Event & evt)
{
  ...
  std::map<int,const simb::MCParticle *> particle_map;
  ...
  if(particle_map.find(1) != particle_map.end() && particle_map.find(2) != particle_map.end()){
    std::cout << "angle: " << TMath::Abs((particle_map.at(1)->Momentum().Vect()).Angle(particle_map.at(2)->Momentum().Vect())) << std::endl;
    if(TMath::Abs((particle_map.at(1)->Momentum().Vect()).Angle(particle_map.at(2)->Momentum().Vect())) > 0.17){
      return true;
    }
  }
